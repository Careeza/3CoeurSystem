...
CS_Scene    *initLevelSelect(SDL_Renderer *render)
{
    ...
    scene->createButtonToSceneFromPng("Continue", "resources/source/FondJeu2.png", 25, 30, 37.5, 35, 5, &goToGame);
    scene->writeTexteScaleW(-1, "Continue");
    scene->addBorder(-1);


    scene->createButtonToSceneFromPng("New", "resources/source/NewGame.png", 25, 30, 7.5, 35, 5, &goToGame);
    scene->writeTexteScaleW(-1, "New Game");
    scene->addBorder(-1);


    scene->createButtonToSceneFromPng("Load", "resources/source/FondJeu2.png", 25, 30, 67.5, 35, 5, &goToGame);
